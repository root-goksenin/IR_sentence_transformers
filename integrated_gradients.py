...
def visualize(attr, tokens, title):    
    return viz.VisualizationDataRecord(
                        ...
                        tokens,
                        0)
def facade(model_before, model_after, query, doc, data_name, qid):
    ...
    query_vis = visualize(q_attr, query_tokens, title = "Before Query")
    doc_vis = visualize(doc_attr, doc_tokens,  title = "Before Doc")
    ...
    query_vis = visualize(q_attr, query_tokens, title = "After Query")
    doc_vis = visualize(doc_attr, doc_tokens, title = "After Doc")
    html = viz.visualize_text([query_vis, doc_vis])
...
def main(data_name):
    ...
    for qid in load_json(f"differences/{data_name}.json"):
        query = [queries[qid]]
